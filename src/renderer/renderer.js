...
function showExePicker(exePaths, installDir, identifier) {
    return new Promise((resolve) => {
        ...
        const titleText = isCollection
            ? (window.i18n?.t('exePicker.collectionTitle') || 'Select a Game to Launch')
            : (window.i18n?.t('exePicker.title') || 'Select Executable');
        const descText = isCollection
            ? (window.i18n?.t('exePicker.collectionDescription') || 'This is a collection — choose which game to play:')
            : (window.i18n?.t('exePicker.description') || 'Multiple executables found. Choose one to launch:');
